...
while (my $window = <$BED>) {
  ...
  my @insert_arr;
  $| = 1;
  open(my $SAM, "samtools view -F1536 -b $bam $region | bedtools intersect -sorted -g $genome -a stdin -b <(echo $window) | samtools view - |");#`bedtools intersect -sorted -g $genome -a $bam -b <(printf "$_") | samtools view - | perl -lane 'if($F[6]eq"="){if($F[8]>500){$insert++};$same++;$total++}else{$total++}END{print "$total\t$same\t".($same/$total)."\t$insert\t".($insert/$total)}'`;
  while (<$SAM>) {
    my @F = split (/\s+/, $_);
    if ($dryrun) {
